...
func (cs *controllerServer) CreateVolume(ctx context.Context, req *csi.CreateVolumeRequest) (*csi.CreateVolumeResponse, error) {
    ...
    if req.GetVolumeContentSource() != nil {
        ...
        if err != nil {
            ...
            if delErr := deleteFlexblockVolume(volumeID); delErr != nil {
                glog.V(2).Infof("deleting flexblock volume %v failed: %v", volumeID, delErr)
            }
            return nil, err
        }
...
func (cs *controllerServer) DeleteVolume(ctx context.Context, req *csi.DeleteVolumeRequest) (*csi.DeleteVolumeResponse, error) {
    ...
    if err := deleteFlexblockVolume(volId); err != nil {
...
// CreateSnapshot uses tar command to create snapshot for flexblock volume. The tar command can quickly create
...
func (cs *controllerServer) CreateSnapshot(ctx context.Context, req *csi.CreateSnapshotRequest) (*csi.CreateSnapshotResponse, error) {
    ...
    flexBlockVolume, ok := flexBlockVolumes[volumeID]
    if !ok {
    ...
    volPath := flexBlockVolume.VolPath
    file := getSnapshotPath(snapshotID)

    var cmd []string
    if flexBlockVolume.VolAccessType == blockAccess {
    ...
    snapshot := flexBlockSnapshot{}
    snapshot.Name = req.GetName()
    snapshot.Id = snapshotID
    snapshot.VolID = volumeID
    snapshot.Path = file
    snapshot.CreationTime = creationTime
    snapshot.SizeBytes = flexBlockVolume.VolSize
    snapshot.ReadyToUse = true

    flexBlockVolumeSnapshots[snapshotID] = snapshot
...
func (cs *controllerServer) DeleteSnapshot(ctx context.Context, req *csi.DeleteSnapshotRequest) (*csi.DeleteSnapshotResponse, error) {
    ...
    os.RemoveAll(path)
    delete(flexBlockVolumeSnapshots, snapshotID)
...
func (cs *controllerServer) ListSnapshots(ctx context.Context, req *csi.ListSnapshotsRequest) (*csi.ListSnapshotsResponse, error) {
    ...
    if len(req.GetSnapshotId()) != 0 {
        ...
        if snapshot, ok := flexBlockVolumeSnapshots[snapshotID]; ok {
    ...
    for k := range flexBlockVolumeSnapshots {
        sortedKeys = append(sortedKeys, k)
    }
    sort.Strings(sortedKeys)

    for _, key := range sortedKeys {
        snap := flexBlockVolumeSnapshots[key]
        snapshot := csi.Snapshot{
            SnapshotId:     snap.Id,
...
func (cs *controllerServer) ControllerExpandVolume(ctx context.Context, req *csi.ControllerExpandVolumeRequest) (*csi.ControllerExpandVolumeResponse, error) {
    ...
    if exVol.VolSize < capacity {
        ...
        if err := updateFlexblockVolume(volID, exVol); err != nil {
...
func convertSnapshot(snap flexBlockSnapshot) *csi.ListSnapshotsResponse {
